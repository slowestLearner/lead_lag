...
for (stock_base in c("all", "large")){
  ...
  process_single_file <- function(file_name, matrix_path, permno_path, msf, crossmom_i, to_dir) {
    ...
    if (file.exists(file_tosave)) return(NULL) # Fail fast
    ...
    for (liq_var in liq_measures) {
      ...
      for (micro_val in c("yes", "no")) {
        ...
        for (part_name in c("total", "asy", "sym")) {
          ...
          # Extract safely
          v_p1 <- agg_res[grp == 1, val]; if(length(v_p1)==0) v_p1 <- 0
          v_p2 <- agg_res[grp == 2, val]; if(length(v_p2)==0) v_p2 <- 0
          v_p3 <- agg_res[grp == 3, val]; if(length(v_p3)==0) v_p3 <- 0
          ...
          l_p1 <- final_lev[grp == 1, final]; if(length(l_p1)==0) l_p1 <- 0
          l_p2 <- final_lev[grp == 2, final]; if(length(l_p2)==0) l_p2 <- 0
          l_p3 <- final_lev[grp == 3, final]; if(length(l_p3)==0) l_p3 <- 0
