...
for (stock_base in c("all", "large")) {
  ...
  # -- get stock returns
  ret_data <- readRDS("../../data/Analyst/news_decomposition_monthly.Rds") %>%
    dplyr::transmute(permno, yyyymm = year(date) * 100 + month(date), cf, dr, sum_news) %>%
    pivot_longer(!c(permno, yyyymm), names_to = "factor_model", values_to = "ret") %>%
    dplyr::select(yyyymm, permno, factor_model, ret) %>%
    setDT()
  
  # ret_data <- readRDS("../../data/Analyst/ltg_chg.Rds") %>%
  #   dplyr::transmute(permno, yyyymm, ltg_chg) %>%
  #   pivot_longer(!c(permno, yyyymm), names_to = "factor_model", values_to = "ret") %>%
  #   dplyr::select(yyyymm, permno, factor_model, ret) %>%
  #   setDT()
  ...
  to_dir_scaling <- to_dir <- paste0("tmp/portfolio_results/", stock_base, "/cf_dr/scaling/")
