    ...
    for (liq_var in liq_measures) {
      ...
      for (micro_val in c("yes", "no")) {
        ...
        for (part_name in c("total", "asy", "sym")) {
          ...
          # Extract values
          val_p1 <- agg_ret[spread_group == 1L, val]
          if (length(val_p1) == 0) val_p1 <- 0
          val_p2 <- agg_ret[spread_group == 2L, val]
          if (length(val_p2) == 0) val_p2 <- 0
          val_p3 <- agg_ret[spread_group == 3L, val]
          if (length(val_p3) == 0) val_p3 <- 0
          ...
          val_p1_lev <- final_lev_grp[spread_group == 1L, final]
          if (length(val_p1_lev) == 0) val_p1_lev <- 0
          val_p2_lev <- final_lev_grp[spread_group == 2L, final]
          if (length(val_p2_lev) == 0) val_p2_lev <- 0
          val_p3_lev <- final_lev_grp[spread_group == 3L, final]
          if (length(val_p3_lev) == 0) val_p3_lev <- 0
      ...
      cols_to_remove <- c(
        "spread_i", "spread_j", "keep_i", "keep_j",
        "ret_t", "spread_group"
      )
