...
for (this_hor in c(1:120)) {
  ...
  ltg_hor <- ltg_hor[, .(permno, yyyymm = 100*year(target_date) + month(target_date),
                         ltg_chg)]
  data <- merge(ltg_hor, signal_data, by = c("yyyymm", "permno")) %>% na.omit()
...
saveRDS(out_all, paste0(to_dir, "dltg_cumulative.RDS"))


out_all <- readRDS(paste0(to_dir, "dltg_cumulative.RDS"))
this_var_type <- "sym"
# this_var_type <- "total"
# this_var_type <- "asy"
...
# # plot
ggplot(out %>%
         dplyr::filter(var == "combined"), aes(x = hor / 12, y = dltg, color = var)) +
  geom_line(lwd = 1) +
  ...
  theme(legend.position = c(.8, .85), legend.title = element_blank(), 
        plot.title = element_text(hjust = .5), text = element_text(size = 30))
