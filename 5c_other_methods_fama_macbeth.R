# Estimates cross-sectional return predictability with Fama-MacBeth regressions under progressively richer controls.
library(this.path)
...
for (stock_base in c("all", "large")) {
  ...
  # compute the combined signal
  data_combined <- data[, .(var = "combined", signal = mean(signal)), .(yyyymm, permno)]
  data <- rbind(data, data_combined)
  rm(data_combined)

  # For comparability, make signals z-scores
  data[, signal := Winsorize(signal, quantile(signal, probs = c(.005, .995))), .(yyyymm, var)]
  ...
  # with parallelization, each var is around 30 secs
