# --- Plot cumulative L/S returns for combined predictor
library(this.path)
...
for (stock_base in c("all", "large")) {
  ...
  # cumulative returns
  data <- readRDS(paste0(from_dir, "scale_by_total.RDS")) %>%
    filter(from_hor == 1) %>%
    filter(to_hor <= 120) %>%
  ...
  for (this_idx in signal_rank[, idx]) {
    # this_idx <- 1
    tic(paste0(stock_base, ", ", this_idx, ", ", signal_rank[idx == this_idx, var]))
