...
for (stock_base in c("all", "large")) {
  ...
  for (h_limit in fill_horizons) {
    ...
    # Update input path to match the directory structure from 3b1
    ...
    data_combined_first <- data_combined[yyyymm <= 196212][, var := "combined_1926_1962"]

    # Late sample: 1963 - 2023
    data_combined_second <- data_combined[yyyymm > 196212][, var := "combined_1963_2023"]
    ...
    # ANCHOR AT T=0
    # ============================================================================
    # Create an anchor at horizon 0 with zero returns for every unique signal/month
    # to ensure plots are continuous through the formation date.
    data_initial <- unique(data[, .(var, var_type, factor_model, yyyymm)]) %>%
      mutate(hor = 0, ret_fut = 0)
    data <- rbind(data, data_initial, fill = TRUE)

    # Identify unique specifications to loop over
    specs <- unique(data[, .(var, var_type, factor_model)])

    # ============================================================================
    # STATISTICAL CALCULATION (NW T-STATS)
    # ============================================================================
    p.get_stats <- function(i, data, specs) {
      ...
      # Loop through every horizon present in the data (e.g., -60 to +120)
      res_list <- lapply(unique(sub_data$hor), function(h) {
        ...
        # NW Lag logic:
        # 1. Use abs(h) because lags cannot be negative.
        # 2. For h=0, lag must be 0 or 1.
        nw_lag <- max(abs(h), 1)

        # prewhite = FALSE prevents VAR(1) crashes in short subsamples
    ...
    # Parallel execution for speed across specifications
